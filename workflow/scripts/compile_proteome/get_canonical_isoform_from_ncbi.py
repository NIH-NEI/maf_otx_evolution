...
1) Read all GFF3 files to build:
   - mRNA -> tags (look for 'tag=RefSeq Select' or 'tag=MANE Select')
   - mRNA -> parent gene (gene ID or locus_tag fallback)
   - mRNA -> CDS protein_id(s)
   - protein_id -> (mRNA, gene)
2) Read protein FASTA(s) to get protein sequences and lengths.
3) Per gene:
   - If any mRNA has a canonical tag, take its protein(s) (typically one).
   - Else pick the longest protein across mRNAs of the gene.
4) Write selected protein FASTA entries, de-duplicated, with informative headers.

Notes:
- NCBI RefSeq GFF3 typically encodes:
  mRNA lines with attributes: ID=..., Parent=gene-..., tag=RefSeq Select|MANE Select (when present)
  CDS lines with attributes: Parent=<mRNA-ID>, protein_id=XP_/NP_/YP_...
- Genes can be addressed by gene ID (ID=) or locus_tag=; we try ID then locus_tag then gene= symbol.
"""
...
def main():
    ...
    for gff in gff_paths:
        with gzip.open(gff, "rt") as fh:
            for line in fh:
                ...
                if ftype == "gene":
                    gid = attrs.get("ID") or attrs.get("locus_tag") or attrs.get("Name") or attrs.get("gene")
                    if gid:
                        # Best-effort store a symbol for nicer headers
                        sym = attrs.get("gene") or attrs.get("Name") or attrs.get("locus_tag") or gid
                ...
                if ftype in ("mRNA", "transcript"):
                    ...
                    if not mid:
                        continue
                    parent_gene = attrs.get("Parent") or attrs.get("gene") or attrs.get("locus_tag")
    ...
    for gid, mrnas in gene_to_mrnas.items():
        ...
        canonical_mrnas = [m for m in mrnas if any(t in ("RefSeq Select", "MANE Select") for t in mrna_tags.get(m, []))]
        candidate_pids = []
        if canonical_mrnas:
            for m in canonical_mrnas:
                candidate_pids.extend(list(mrna_to_protein.get(m, [])))
        else:
            # 2) Fallback: longest protein across all mrnas in this gene
            all_pids = []
            for m in mrnas:
                all_pids.extend(list(mrna_to_protein.get(m, [])))
            if all_pids:
    ...
    with open(outp, "w") as out_fh:
        for (gid, pid) in sorted(unique.keys()):
            ...
            # Header example:
            # >pid|org=human|gene=SYMBOL|gid=gene-123|assembly=GCF_...|source=RefSeq
            #header = f"{pid}|org={org}|gene={sym}|gid={gid}|dbxref={ref}|assembly={acc}|source={src}"
